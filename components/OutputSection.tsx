    ...
    const renderContent = () => {
        switch (processingState) {
            case ProcessingState.IDLE:
                return <div className="text-center text-gray-400">{t('outputSection.idle')}</div>;
            case ProcessingState.ERROR:
                 return <div className="text-center text-red-400">{t('outputSection.error')}</div>;
            ...
            case ProcessingState.SYNTHESIZING:
                return (
                    <div className="flex flex-col items-center justify-center h-full text-gray-400">
                        <Spinner size="lg" />
                        <p className="mt-4">{t('outputSection.generating')}</p>
                    </div>
                );
            case ProcessingState.DONE:
                if (articles.length === 0) {
                    return <div className="text-center text-gray-400">{t('outputSection.noArticles')}</div>;
                }
                return (
                            ...
                            <Card key={index} className="bg-gray-800/50">
                                <div className="p-5">
                                    <div className="flex justify-between items-center mb-3">
                                        ...
                                        <button onClick={() => handleDownload(article)} className="p-2 rounded-full text-gray-400 hover:text-white hover:bg-gray-700 transition-colors" aria-label={t('outputSection.downloadArticle')}>
    ...
    return (
        <Card className="h-full">
            <div className="p-6 h-full flex flex-col">
                <h2 className="text-lg font-semibold text-white mb-4 flex-shrink-0">{t('outputSection.title')}</h2>
